...
(function ($) {
	...
	// ==============================================
	// Safe play/pause helpers for hover-preview videos
	// Prevents "AbortError: play() interrupted by pause()"
	// when the pointer leaves before play() has resolved.
	// ==============================================
	...
	function pauseVideoSafe(video) {
		if (video._playPromise !== undefined) {
			video._playPromise.then(function() {
				video.pause();
			}).catch(function() {});
		} else {
			video.pause();
	...
	$(".pgi-image-wrap").on("mouseenter touchstart", function() {
		$(this).find("video").each(function() {
			playVideoSafe($(this).get(0));
		}); 
	}).on("mouseleave touchend", function() {
		$(this).find("video").each(function() {
			pauseVideoSafe($(this).get(0));
		});
	});
